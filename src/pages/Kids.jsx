...
function Kids() {
  ...
  const fetchKidsProducts = async () => {
    ...
    try {
      ...
      const kidsProducts = allProducts.filter(
        (product) => product.cat_name?.cat_name.toLowerCase() === "kids"
      );
  ...
  return (
    <>
      ...
      />
      <BgBody>
        <h2 className="text-xl font-medium mb-4">Kid's Shoes : </h2>
          ...
          <div className=" grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {products.map((product) => (
              <Link to={`/product/${product._id}`} key={product._id}>
                <div
                  key={product._id}
                  className="bg-white rounded-xl py-1 px-4 shadow hover:shadow-xl transition-all "
                >
